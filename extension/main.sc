...
(pre-define imht-set-key-t int imht-set-can-contain-zero? #f)
...
(pre-define
  ...
  (do-while (= errno EINTR) (set a (dup a))) (dup2-fd old new)
  (begin (do-while (= errno EINTR) (dup2 old new)) (close old)) (close-fd scm a)
  (if (= scm SCM-BOOL-F) (close a)) (ensure-fd a open-flags path)
  (begin
...
(define (scm-list->imht-set scm-a result) (int SCM imht-set-t**)
  ...
  (while (not (scm-is-null scm-a))
    (if (not (imht-set-add *result (scm->int (SCM-CAR scm-a))))
      (begin (imht-set-destroy *result) (return 2)))
...
(define
  (scm-primitive-process-create scm-executable scm-arguments
  ...
  (declare keep imht-set-t* input int output int error int)
