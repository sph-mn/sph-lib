...
modules under highlights receive most attention and all issues are tried to be fixed immediately. other modules are to be seen as pools of examples and are deemed less mature or less useful, features are more likely to be removed.
...
the extension exists because guile does not come with a generic process creation procedure and it seems that it can not be implemented reliably otherwise.
