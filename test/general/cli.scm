...
(define test-env-cli-config
  (list #:name "test-interface"
    ...
    #:version (ql 0 1) #:help "test help" #:about "-my-custom-about-text-"
    #:missing-arguments-handler #f
    ))

(define test-env-cli (apply cli-create test-env-cli-config))
(define (test-general inp) (test-env-cli "a" "b" "c" "-i" "1"))

(define (test-required)
  (assert-and
    (assert-true "named"
      (catch (q missing-arguments) (l () (test-env-cli))
        (l (key count missing) (equal? (ql input) missing)))))
  (assert-true "unnamed"
    (catch (q missing-arguments) (l () (test-env-cli "-i" "1"))
