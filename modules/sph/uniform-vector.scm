(define-module (sph uniform-vector))
(import (rnrs bytevectors) (sph) (sph number) (srfi srfi-4) (srfi srfi-1))

(export bytevector-append bytevector-contains?
  f32vector-copy f32vector-copy*
  f32vector-copy-zero f32vector-copy-zero*
  f32vector-create f32vector-each-index
  f32vector-map f32vector-map!
  f32vector-map-with f32vector-map-with!
  f32vector-map-with-index f32vector-map-with-index!
  f32vector-range-map f32vector-range-map!
  f32vector-range-map-with f32vector-range-map-with!
  f32vector-range-map-with-index! f32vector-range-set
  f32vector-range-set! f64vector-copy
  f64vector-copy* f64vector-copy-zero
  f64vector-copy-zero* f64vector-create
  f64vector-each-index f64vector-map
  f64vector-map! f64vector-map-with
  f64vector-map-with! f64vector-map-with-index
  f64vector-map-with-index! f64vector-range-map
  f64vector-range-map! f64vector-range-map-with
  f64vector-range-map-with! f64vector-range-map-with-index!
  f64vector-range-set f64vector-range-set!
  integer->bytevector s16vector-copy
  s16vector-copy* s16vector-copy-zero
  s16vector-copy-zero* s16vector-create
  s16vector-each-index s16vector-map
  s16vector-map! s16vector-map-with
  s16vector-map-with! s16vector-range-map
  s16vector-range-map! s16vector-range-map-with
  s16vector-range-map-with! s16vector-range-set
  s16vector-range-set! s32vector-copy
  s32vector-copy* s32vector-copy-zero
  s32vector-copy-zero* s32vector-create
  s32vector-each-index s32vector-map
  s32vector-map! s32vector-map-with
  s32vector-map-with! s32vector-range-map
  s32vector-range-map! s32vector-range-map-with
  s32vector-range-map-with! s32vector-range-set
  s32vector-range-set! s8vector-copy
  s8vector-copy* s8vector-copy-zero
  s8vector-copy-zero* s8vector-create
  s8vector-each-index s8vector-map
  s8vector-map! s8vector-map-with
  s8vector-map-with! s8vector-range-map
  s8vector-range-map! s8vector-range-map-with
  s8vector-range-map-with! s8vector-range-set
  s8vector-range-set! sph-uniform-vector-description
  u32vector-copy u32vector-copy*
  u32vector-copy-zero u32vector-copy-zero*
  u32vector-create u32vector-each-index
  u32vector-map u32vector-map!
  u32vector-map-with u32vector-map-with!
  u32vector-range-map u32vector-range-map!
  u32vector-range-map-with u32vector-range-map-with! u32vector-range-set u32vector-range-set!)
...
(define-syntax-rule (define-uv-create id make-vector vector-set!)
  (define (id length f)
    ...
    (let (result (make-vector length))
      (let loop ((index 0))
        (if (< index length) (begin (vector-set! result index (f index)) (loop (+ 1 index))) result)))))
...
(define-syntax-rule (define-uv-map-with-index id vector-copy-zero* vector-range-map! vector-length)
  (define (id f a . b)
...
(define (integer->bytevector a)
  ...
  (let*
    ((size (bit->byte-length (+ 1 (number-container-length (abs a) 2)))) (r (make-bytevector size)))
