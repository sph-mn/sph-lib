...
(export alist->record define-record
  define-record-accessors define-record-setters
  make-record make-record-layout
  record record->vector
  record-accessors record-append
  record-field-names record-field-names-unordered
  record-layout->predicate record-layout-extend!
  record-layout-length record-layout-merge
  record-layout-merge! record-layout?
  record-length record-list-filter-value
  record-ref record-set
  record-setter record-setters
...
(define (make-record record-layout) "record-layout -> record" (make-vector (ht-size record-layout)))
...
(define (vector->record record-layout a)
  ...
  (let ((record-layout-size (ht-size record-layout)) (vec-size (vector-length a)))
    (if (eqv? record-layout-size vec-size) a
      (if (< vec-size record-layout-size) (vector-extend a (- record-layout-size vec-size)) a))))
...
(define-syntax-case (define-record name field-name/get/set ...) s
  (let
    ...
    (datum->syntax s
      (pairs (q begin)
        (list (q define) name (list (q make-record-layout) (list (q quote) (map first field-spec))))
        (fold
          (l (e r)
            (apply
