(define-module (sph io path-pipe-chain))
(use-modules (ice-9 threads) (sph) (sph io) (sph other) (sph vector) (srfi srfi-1))
(export path-pipe-chain sph-io-path-pipe-chain-description)
          ...
          (map-last-output
            (l (current next out c)
              ...
              (case (join-symbols current next)
                ...
                ((port-path) (c (open out (logior O_WRONLY O_CREAT)) #f))
                (else (c out #f)))))
          (map-output-input
            (l (current next c)
              ...
              (case (join-symbols current next)
                ...
                ((path-path) (let (path (named-pipe)) (c path path)))
                (else (c #f #f))))))
        (l (first-input last-output . config)
