(library (sph module)
              ...
              (call-with-input-file path
                (l (file)
                  (if file-content-match (file-content-match file)
                    (or
                      (let (r (module-match-rnrs-definition (read file))) (seek file 0 SEEK_SET) r)
                      (and guile-modules?
  ...
  (define*
    ...
    (fold-directory-tree
      (l (sub-path stat-info r)
        (let
          (name
            (module-find-one sub-path #:load-path
  ...
  (define (import-any . module-names)
    ...
    (any
      (l (e)
        (if (module-exists-path? e) (begin (module-use! (current-module) (resolve-interface e)) #t)
          #f))
      module-names))
  ...
  (define*
    (import-directory-tree path #:key (resolve-interface-args (list)) #:rest module-find-arguments)
    "string #:key (max-depth integer) (resolve-interface-args list/procedure) -> unspecified
     imports libraries with module-find"
    (let (current-module* (current-module))
      (each
        ...
        (apply module-find path module-find-arguments))))
  ...
  (define (import-if-existent . module-names)
    (map (l (e) (module-use! (current-module) (resolve-interface e)))
      (filter module-exists-path? module-names)))
  ...
  (define (module-exists-path? name) "(symbol ...) -> boolean"
  ...
  (define (file-contains-module? path)
    ...
    (if (module-name-from-file path) #t #f))

  (define (module-name-from-file a)
    "string -> (symbol ...)/false
  ...
  (define (path->symbol-list a)
    "create a module name from a filesystem path string. for example \"/a/b/c\" -> (a b c)"
