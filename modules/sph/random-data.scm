(library (sph random-data)
  ...
  (import
    ...
    (rnrs bytevectors)
    (sph)
    (only (guile) random-state-from-platform char-set:designated)
    (only (sph list) map-integers)
    (rename (guile) (random primitive-random)))

  (define sph-random-data-description "generate random data. strings, booleans, lists, and more")
  ...
  (define*
    ...
    (let*
      ...
      (map-integers list-length (l (n) (vector-ref a (random a-last-index 0 state))))))
