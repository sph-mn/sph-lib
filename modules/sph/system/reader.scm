(define-module (sph system reader))

(use-modules (ice-9 peg) (rnrs base)
  (sph) (system reader)
  ((ice-9 rdelim) #:select (read-line)) ((sph io) #:select (read-until-string-proc))
  ((srfi srfi-1) #:select (reverse)))

(export default-sharp-token-reader-with-comments read-for-formatting
  read-with-comments sph-read
  sph-read-sharp-token-readers sph-read-token-readers
...
(define (read-range-expr->list port start end proc)
  ...
  (let loop ((char (read-char port)) (level (list)))
    (if (or (eqv? end char) (eof-object? char)) (reverse level)
      (let
        (level
          (if (eqv? start char) (pair (loop (read-char port) (list)) level) (proc char port level)))
