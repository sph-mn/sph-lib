...
(export sph-sql-description sql-columns
  ...
  sql-create-table sql-delete
  sql-delete-table sql-insert
  sql-insert-values sql-select sql-string sql-update sql-value sql-where sql-where-condition)
...
(define (operator->sql-operator a)
...
(define (sql-where-column-expr-produce-proc column-name state->combinator)
  "this procedure might be doing too much at once, split if possible"
  (let*
    ( (join-values
                    ...
                    (if (= 1 (length columns))
                      ...
                      (parenthesise
                        (string-join
                          (map
                            (l (c)
                              (if (list? c) (first c) (join-values c values operator sql-operator)))
                            columns)
                          column-combinator))))))
...
(define primitive-sql-where-condition
  (let*
    ( (row-expr-binary
      ...
      (column-expr
        (l (prefix suffix) "add implicit operator to column-expression"
        ...
        (l (prefix suffix level)
          ( (case prefix
            ...
            (if (pair? (first suffix))
              (map (l (a) (if (pair? a) (join-column-value (first a) "=" (tail a)) a)) suffix) suffix)
            level))))
...
(define (sql-options options)
  ...
  (alist-bind options (order limit offset begin end)
    (pair (if begin (string-append " " begin) "")
      (string-append
        (if offset (string-append " offset " (if (integer? offset) (number->string offset) offset))
          "")
        (if limit
          (cond
...
(define* (sql-delete table-name #:optional where options) "string sql-where sql-options -> string"
