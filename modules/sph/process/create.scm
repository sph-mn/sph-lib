...
(define*
  (process-create executable #:optional (arguments (list)) input-port output-port error-port #:key
    env
  ...
  (if search-path?
    (let
      (executable (if (string-prefix? "/" executable) executable (search-env-path-one executable)))
      ...
      (let*
        ( (ports
            (let loop
              ...
              (if (null? ports) (list)
                (if (first ports)
                  (pair (if first? (first pipes) (pair-reverse (first pipes)))
    ...
    ( (config-length (length config)) (error (current-error-port))
      (pids
        (apply path-pipe-chain first-input
          last-output
          (map
            (l (a) "creates a path-pipe-chain configuration"
