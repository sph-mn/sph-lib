...
(library (sph web html-include)
    ...
    (any
      (l (ele)
        (let ((full-path (string-append path (first ele))))
          (if (file-exists? full-path) (cons full-path (tail ele)) #f)))
      suffix->handler))

  (define (find-path+search-path proc paths search-paths)
    (every-map (l (path) (any (l (search-path) (proc search-path path)) search-paths)) paths))

  (define (find-files paths search-paths suffix->handler)
    (find-path+search-path
      (l search-path+path (find-file (apply string-append search-path+path) suffix->handler)) paths
      search-paths))
  ...
  (define (file-update-necessary? target-path found-files) "-> boolean"
    (if (file-exists? target-path)
      (let
        ( (files-newest-mtime
            (apply max (map (l (ele) (stat:mtime (stat (first ele)))) found-files)))
  ...
  (define (paths->target-file-name arg suffix)
    (string-append (remove-filename-extension (last arg)) "-"
      (number->string (string-hash (apply string-append arg)) 32) suffix))
      ...
      (if found-files
        (let*
          ...
          (if #t
            ...
            (begin
              (apply merge-files
                (begin (ensure-directory-structure (dirname target-path)) target-path)
  ...
  (define (html-include-proc include-config content-type->search-paths)
    ...
    (lambda (content-type . paths)
      (let
        (path
          (prepare-include (hashtable-ref include-config content-type)
            (content-type->search-paths content-type) (string-join (map symbol->string paths) "/")))
        (if path ((content-type->sxhtml-include-tag content-type) path) "")))))
