(library (sph time gregorian)
  ...
  (define (greg-years->leap-days a)
    "number of leap days before the year was reached"
    (if (< a 4) 0
      (- (quotient a 4) (- (quotient a 100) (quotient a 400)))))
  ...
  (define (greg-years->days a)
    "elapsed days to reach given year"
  ...
  (define (greg-month->days a leap-year?)
    ...
    (let ((month-days (greg-month-days-get leap-year?)) (end (- a 1)))
      (let loop ((index 0) (days 0))
        (if (< index end) (loop (+ 1 index) (+ days (vector-ref month-days index)))
          days))))
