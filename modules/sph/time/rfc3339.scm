(define-module (sph time rfc3339))

(use-modules (srfi srfi-1) (ice-9 peg)
  (sph) ((sph alist) #:select (alist-ref alist-q))
  ((sph string) #:select (string-fill-left string-equal?))
  ((sph time) #:select (nanoseconds->seconds utc-from-date date-new seconds->nanoseconds))
  ((sph time utc) #:select (utc-duration->hms)) ((sph tree) #:select (tree-splice))
  ((srfi srfi-1) #:select (drop-right)))

(export sph-time-rfc3339-description time-rfc3339->alist
  time-rfc3339-parse& time-rfc3339-parse-tree utc->rfc3339 utc-from-rfc3339 utc-ns-from-rfc3339)
...
(define (rfc3339->alist a) "string -> list/false"
  (rfc3339-parse& a
    (l
      (year month day hours minutes seconds seconds-fraction offset-negative? offset-hours
        offset-minutes)
      (alist-q year year
        month month
        day day
