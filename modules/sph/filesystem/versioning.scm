(define-module (sph filesystem versioning))

(use-modules (sph) (sph hashtable)
  (sph io) (sph process) ((sph filesystem) #:select (ensure-directory-structure directory-list)))

(export sph-filesystem-versioning-description versioning-create
  versioning-default-config versioning-restore)
...
(define versioning-default-config
  (ht-create-symbol-q max-count 3 max-size (inf) path-versions "versions/" path-temp "temp/"))
...
(define (delete-oldest-versions after-version-id existing-version-ids path-versions)
  ...
  (each
    (l (e)
      (if (< e after-version-id) (delete-file (string-append path-versions (number->string e 16)))))
          ...
          (if (>= v version-id)
            (begin
              (execute "patch" "--unified"
                "--silent" "--reject-file=-" restored-path (path-version-file-string path-versions v))
              (loop (- v 1)))))
        restored-path))))
