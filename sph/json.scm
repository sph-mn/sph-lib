(library (sph json)
  ...
  (define (hashtable->json a port) (display "{" port)
    (hashtable-each
  ...
  (define-syntax-rule (boolean->json a port) (display (if a "true" "false") port))

  (define* (scm->json a #:optional (port (current-output-port)))
  ...
  (define (scm->json-string a) (call-with-output-string (l (port) (scm->json a port)))))