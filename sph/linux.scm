(library (sph linux)
  ...
  (define (utsname:parsed-linux-version sys-info)
    ...
    (map string->number
      (string-split (first (string-split (utsname:release sys-info) #\-)) #\.)))
  ...
  (define (inotify-watch paths events proc)
    ...
    (let*
      ( (inotify (inotify-init))
        (watch-ide
          (if (list? paths)
            (map (l (path) (inotify-add-watch inotify path events)) paths)
            (inotify-add-watch inotify paths events))))
      (letrec ((loop (l () (proc (inotify-read inotify)) (loop))))
        (loop)))))
